...
function SubscriptionTracker() {
  ...
  const toggleStatus = (id) => {
    ...
    if (subscription.status === "Cancelled") {
      toast.info("Cancelled subscriptions cannot be resumed.");
      return;
    }
  ...
  const toggleAutoRenew = (id) => {
    ...
    if (subscription.status !== "Active") {
      toast.info("Only active subscriptions can use auto-renew.");
      return;
    }
    ...
    setSubscriptions((current) =>
      current.map((item) =>
        item.id === id ? { ...item, autoRenew: newValue } : item
      )
    );
  ...
  const openRenewalReview = () => {
    setFilter("Active");
    setSearch("");
    toast.info("Showing active subscriptions for renewal review.");
  };
  ...
  const filteredSubscriptions = useMemo(() => {
    return subscriptions.filter((subscription) => {
      ...
      const matchesSearch =
        ...
        subscription.category.toLowerCase().includes(searchValue);

      const matchesFilter =
        filter === "All" || subscription.status === filter;
  ...
  const getMonthlyCost = (item) => {
    ...
    if (item.cycle === "Weekly") return item.amount * 4.345;
    return item.amount;
  };
  ...
  const monthlyCost = activeSubscriptions.reduce(
    (total, item) => total + getMonthlyCost(item),
    0
  );
  ...
  const upcomingPayments = activeSubscriptions
    .filter((item) => {
      const days = getDaysDifference(item.nextPayment);
  ...
  const dueSoonCount = activeSubscriptions.filter((item) => {
    const days = getDaysDifference(item.nextPayment);
    return days !== null && days >= 0 && days <= 3;
  }).length;

  const overdueCount = activeSubscriptions.filter((item) => {
    const days = getDaysDifference(item.nextPayment);
    return days !== null && days < 0;
  }).length;
  ...
  const categoryInsights = useMemo(() => {
    ...
    activeSubscriptions.forEach((subscription) => {
      ...
      categories[category].monthly += getMonthlyCost(subscription);
      categories[category].count += 1;
      categories[category].subscriptions.push(subscription);
    });
    ...
    return Object.values(categories)
      .map((item) => ({
        ...item,
        percentage:
          monthlyCost > 0 ? (item.monthly / monthlyCost) * 100 : 0,
      }))
      .sort((a, b) => b.monthly - a.monthly);
  }, [activeSubscriptions, monthlyCost]);

  const highestCategory =
    categoryInsights.length > 0 ? categoryInsights[0] : null;

  const next7DaysCount = activeSubscriptions.filter((item) => {
    const days = getDaysDifference(item.nextPayment);
    return days !== null && days >= 0 && days <= 7;
  }).length;

  const next30DaysSubscriptions = activeSubscriptions.filter((item) => {
    const days = getDaysDifference(item.nextPayment);
    return days !== null && days >= 0 && days <= 30;
  });

  const next30DaysCount = next30DaysSubscriptions.length;

  const next30DaysRenewalSpend = next30DaysSubscriptions.reduce(
  ...
  const categoryAverage =
    categoryInsights.length > 0
      ? monthlyCost / categoryInsights.length
      : 0;
  ...
  const renewalSubscriptions = activeSubscriptions
    .filter((item) => item.nextPayment)
    .map((item) => ({
      ...item,
      renewalDays: getDaysDifference(item.nextPayment),
      renewalInfo: getRenewalInfo(item.nextPayment),
    }))
    .sort((a, b) => a.renewalDays - b.renewalDays);

  const renewalWithin7Days = renewalSubscriptions.filter(
    (item) =>
  ...
  const renewalWithin30Days = renewalSubscriptions.filter(
    (item) =>
      item.renewalDays !== null &&
      item.renewalDays >= 0 &&
      item.renewalDays <= 30
  ...
  const renewalWithin90Days = renewalSubscriptions.filter(
    (item) =>
      item.renewalDays !== null &&
      item.renewalDays >= 0 &&
      item.renewalDays <= 90
  ...
  const renewalSpend7Days = renewalWithin7Days.reduce(
    (total, item) => total + item.amount,
    0
  );

  const renewalSpend30Days = renewalWithin30Days.reduce(
    (total, item) => total + item.amount,
    0
  );

  const renewalSpend90Days = renewalWithin90Days.reduce(
    (total, item) => total + item.amount,
    0
  );

  const overdueRenewals = renewalSubscriptions.filter(
    (item) => item.renewalDays < 0
  );

  const todayRenewals = renewalSubscriptions.filter(
    (item) => item.renewalDays === 0
  );

  const tomorrowRenewals = renewalSubscriptions.filter(
    (item) => item.renewalDays === 1
  );

  const soonRenewals = renewalSubscriptions.filter(
    (item) => item.renewalDays >= 2 && item.renewalDays <= 7
  );

  const autoRenewUpcoming = renewalWithin30Days.filter(
    (item) => item.autoRenew
  );

  const nextRenewalIntelligence =
    renewalSubscriptions.find((item) => item.renewalDays >= 0) || null;

  const renewalWindowSubscriptions = renewalSubscriptions.filter(
    (item) =>
      item.renewalDays !== null &&
      item.renewalDays >= 0 &&
      item.renewalDays <= renewalWindow
  );

  const renewalWindowSpend = renewalWindowSubscriptions.reduce(
    (total, item) => total + item.amount,
    0
  );

  const renewalWindowAutoRenew = renewalWindowSubscriptions.filter(
    (item) => item.autoRenew
  ).length;

  const renewalWindowManual = renewalWindowSubscriptions.filter(
    (item) => !item.autoRenew
  ).length;
  ...
  const autoRenewRate =
    activeSubscriptions.length > 0
      ? (autoRenewCount / activeSubscriptions.length) * 100
      : 0;
  ...
  const healthScore = Math.max(
    ...
    Math.min(
      100,
      100 - overdueCount * 20 - dueSoonCount * 6 - pausedCount * 4
    )
  );
  ...
  if (highestCostSubscription) {
    smartInsights.push({
      type: "spending",
      icon: <FaTrophy />,
      title: "Highest recurring cost",
      text: `${highestCostSubscription.name} accounts for ${formatCurrency(
        getMonthlyCost(highestCostSubscription)
  ...
  if (highestCategory) {
    smartInsights.push({
      ...
      text: `${highestCategory.category} represents ${highestCategory.percentage.toFixed(
        0
  ...
  if (dueSoonCount > 0) {
    smartInsights.push({
      type: "renewal",
      icon: <FaClock />,
      title: "Renewals coming soon",
      text: `${dueSoonCount} active subscription${
        dueSoonCount !== 1 ? "s" : ""
      } ${dueSoonCount !== 1 ? "are" : "is"} due within the next 3 days.`,
      className: "orange",
    });
  } else if (nextPayment) {
    smartInsights.push({
      ...
      icon: <FaCalendarAlt />,
      title: "Next renewal",
      text: `${nextPayment.name} is your next scheduled payment on ${formatDate(
  ...
  if (autoRenewCount > 0) {
    smartInsights.push({
      ...
      } ${activeSubscriptions.length !== 1 ? "have" : "has"} auto-renew enabled.`,
      className: "cyan",
    });
    ...
    smartInsights.push({
      type: "auto-renew",
      icon: <FaSyncAlt />,
      title: "Auto-renewal review",
      text: "None of your active subscriptions have auto-renew enabled.",
      className: "violet",
    });
  }

  if (monthlyCost > 0 && highestCostSubscription) {
    const highestMonthly = getMonthlyCost(highestCostSubscription);
    const share = (highestMonthly / monthlyCost) * 100;

    if (share >= 40) {
      smartInsights.push({
        type: "review",
        icon: <FaWallet />,
        title: "Large spending concentration",
        text: `${highestCostSubscription.name} makes up ${share.toFixed(
          0
  ...
  const limitedSmartInsights = smartInsights.slice(0, 5);
  ...
  return (
    <div className="dashboard subscription-page">
        ...
        <main className="dashboard-main subscription-main">
          ...
          <section className="subscription-summary">
            ...
            <div className="subscription-summary-card">
              ...
              <div className="subscription-summary-content">
                ...
                <small>Estimated recurring spend</small>
              </div>
            </div>
            ...
            <div className="subscription-summary-card">
              ...
              <div className="subscription-summary-content">
                ...
                <strong>{formatCurrency(yearlyCost)}</strong>
            ...
            <div className="subscription-summary-card">
              ...
              <div className="subscription-summary-content">
                ...
                <strong>{nextPayment ? formatCurrency(nextPayment.amount) : "₹0"}</strong>
                <small
                  className={
                    nextPayment
                      ? `renewal-text ${getRenewalInfo(nextPayment.nextPayment).className}`
                      : ""
                  }
                >
                  {nextPayment
                    ? getRenewalInfo(nextPayment.nextPayment).label
                    : "No upcoming payment"}
                </small>
              </div>
            </div>
            ...
            <div className="subscription-summary-card">
              ...
              <div className="subscription-summary-content">
                ...
                <strong className="summary-name">
                  {highestCostSubscription ? highestCostSubscription.name : "None"}
                </strong>
                <small>
                  {highestCostSubscription
                    ? `${formatCurrency(getMonthlyCost(highestCostSubscription))} monthly`
            ...
            <div className="subscription-summary-card">
              ...
              <div className="subscription-summary-content">
                ...
                <strong className="summary-name">
                  {nextPayment ? nextPayment.name : "None"}
                </strong>
                <small>
                  {nextPayment
                    ? formatDate(nextPayment.nextPayment)
            ...
            <div className="subscription-summary-card">
              ...
              <div className="subscription-summary-content">
                ...
                <strong>{autoRenewCount}</strong>
                <small>Active auto-renewals</small>
              </div>
            </div>
          </section>
          ...
          {(dueSoonCount > 0 || overdueCount > 0) && (
            <section className="subscription-renewal-alert">
              <div className="subscription-renewal-alert-icon">
                {overdueCount > 0 ? <FaExclamationTriangle /> : <FaClock />}
              </div>
              ...
              <div className="subscription-renewal-alert-content">
                <strong>
                  {overdueCount > 0
                    ...
                    : `${dueSoonCount} payment${dueSoonCount !== 1 ? "s" : ""} due soon`}
                </strong>
            ...
            <div className="subscription-category-header">
              ...
              {highestCategory && (
                <div className="subscription-category-highlight">
                  ...
                  <small>
                    {formatCurrency(highestCategory.monthly)}/month
                  </small>
                </div>
              )}
            </div>
            ...
            {categoryInsights.length === 0 ? (
              ...
              <div className="subscription-category-grid">
                {categoryInsights.map((item) => {
                  const categoryClass =
                    categoryClasses[item.category] || "other";
          ...
          <section className="subscription-overview-section">
            <div className="subscription-overview-header">
              <div className="subscription-overview-title">
                ...
                <div>
                  <h2>Spending &amp; Renewal Overview</h2>
                  <p>A quick look at your recurring financial commitments.</p>
                </div>
              </div>
            </div>
            ...
            <div className="subscription-overview-grid">
              <div className="subscription-overview-card blue">
                <div className="subscription-overview-icon"><FaWallet /></div>
                <div className="subscription-overview-content">
                  ...
                  <p>Estimated recurring monthly cost</p>
                </div>
              </div>

              <div className="subscription-overview-card purple">
                <div className="subscription-overview-icon"><FaChartLine /></div>
                <div className="subscription-overview-content">
                  <div className="subscription-overview-card-top">
                    <div>
                  ...
                  <p>Projected yearly subscription spending</p>
                </div>
              </div>

              <div className="subscription-overview-card orange">
                <div className="subscription-overview-icon"><FaClock /></div>
                <div className="subscription-overview-content">
                  <div className="subscription-overview-card-top">
                    <div>
                    ...
                    <strong>{next7DaysCount}</strong>
                  </div>
                  <p>Subscription renewals within 7 days</p>
                </div>
              </div>

              <div className="subscription-overview-card cyan">
                <div className="subscription-overview-icon"><FaCalendarAlt /></div>
                <div className="subscription-overview-content">
                  <div className="subscription-overview-card-top">
                    <div>
                      <span>Next 30 Days</span>
                      <small>Renewal count</small>
                    </div>
                    <strong>{next30DaysCount}</strong>
                  </div>
                  <p>Renewals expected within 30 days</p>
                </div>
              </div>

              <div className="subscription-overview-card green">
                <div className="subscription-overview-icon"><FaCreditCard /></div>
                <div className="subscription-overview-content">
                  <div className="subscription-overview-card-top">
                    <div>
                      <span>Renewal Spend</span>
                      <small>Next 30 days</small>
                    </div>
                    <strong>{formatCurrency(next30DaysRenewalSpend)}</strong>
                  </div>
                  <p>Expected payment amount from upcoming renewals</p>
                </div>
              </div>
              ...
              <div className="subscription-overview-card violet">
                ...
                <div className="subscription-overview-content">
                  <div className="subscription-overview-card-top">
                    ...
                    <strong>{formatCurrency(categoryAverage)}</strong>
                  </div>
                  <p>Average monthly spending per category</p>
                </div>
              </div>
            </div>
          </section>
            ...
            <div className="subscription-health-card">
              <div className="subscription-health-header">
                <div className="subscription-health-title">
                  <div className={`subscription-health-icon ${healthClass}`}>
                    {healthScore >= 75 ? <FaCheckCircle /> : <FaExclamationTriangle />}
                  </div>

                  <div>
                    <h2>Subscription Health</h2>
                    <p>A quick view of how healthy your recurring payments are.</p>
                  </div>
                </div>

                <div className={`subscription-health-score ${healthClass}`}>
              ...
              <div className="subscription-health-progress">
                ...
                <div className="subscription-health-status-row">
                  <strong className={healthClass}>{healthStatus}</strong>
                  <span>
                    {overdueCount > 0
                      ? `${overdueCount} overdue payment${overdueCount !== 1 ? "s" : ""}`
                      : dueSoonCount > 0
                        ? `${dueSoonCount} renewal${dueSoonCount !== 1 ? "s" : ""} due soon`
              ...
              <div className="subscription-health-metrics">
                <div><span>ACTIVE</span><strong>{activeSubscriptions.length}</strong></div>
                <div><span>AUTO-RENEW</span><strong>{Math.round(autoRenewRate)}%</strong></div>
              ...
              <div className="subscription-insights-header">
                <div className="subscription-insights-title">
                  ...
                  <div>
                    ...
                    <p>Useful observations from your subscription activity.</p>
                  </div>
                </div>

                <span className="subscription-insights-badge">SMART</span>
              </div>

              {limitedSmartInsights.length === 0 ? (
                <div className="subscription-insights-empty">
                  <FaChartLine />
                  <p>Add active subscriptions to unlock smart insights.</p>
                </div>
              ) : (
                <div className="subscription-insights-list">
                  {limitedSmartInsights.map((insight, index) => (
                    <div
                      className="subscription-insight"
                      key={`${insight.type}-${index}`}
                    >
                      <div className={`subscription-insight-icon ${insight.className}`}>
                        {insight.icon}
                      </div>
          ...
          <section className="subscription-renewal-intelligence">
            <div className="subscription-renewal-intelligence-header">
              ...
              <div className="subscription-renewal-smart-badge">SMART</div>
            </div>
            ...
            <div className="subscription-renewal-intelligence-grid">
              <div className="subscription-renewal-next">
                <div className="subscription-renewal-next-top">
                  <div>
                    <span className="subscription-renewal-label">NEXT RENEWAL</span>
                    <h3>
                ...
                {nextRenewalIntelligence ? (
                  <>
                    <div className="subscription-renewal-next-details">
                      <div>
                        <span>AMOUNT</span>
                        <strong>{formatCurrency(nextRenewalIntelligence.amount)}</strong>
                      </div>
                      <div>
                        <span>DATE</span>
                        <strong>{formatDate(nextRenewalIntelligence.nextPayment)}</strong>
                      </div>
                      <div>
                        <span>PAYMENT</span>
                        <strong>{nextRenewalIntelligence.paymentMethod}</strong>
                      </div>
                    </div>
                    ...
                    >
                      {nextRenewalIntelligence.renewalInfo.icon}
                      <span>{nextRenewalIntelligence.renewalInfo.label}</span>
                    </div>
                  </>
                ) : (
              ...
              <div className="subscription-renewal-spend">
                <div className="subscription-renewal-card-heading">
                  <div className="subscription-renewal-small-icon blue"><FaWallet /></div>
                  <div>
                    <h3>Upcoming Renewal Spend</h3>
                    <p>Expected subscription charges</p>
                  </div>
                </div>
                ...
                <div className="subscription-renewal-spend-grid">
                  <div>
                    <span>NEXT 7 DAYS</span>
                    <strong>{formatCurrency(renewalSpend7Days)}</strong>
                    <small>{renewalWithin7Days.length} renewal{renewalWithin7Days.length !== 1 ? "s" : ""}</small>
                  </div>

                  <div>
                    <span>NEXT 30 DAYS</span>
                    <strong>{formatCurrency(renewalSpend30Days)}</strong>
                    <small>{renewalWithin30Days.length} renewal{renewalWithin30Days.length !== 1 ? "s" : ""}</small>
                  </div>

                  <div>
                    <span>NEXT 90 DAYS</span>
                    <strong>{formatCurrency(renewalSpend90Days)}</strong>
                    <small>{renewalWithin90Days.length} renewal{renewalWithin90Days.length !== 1 ? "s" : ""}</small>
                  </div>
                </div>
              </div>
              ...
              <div className="subscription-renewal-urgency">
                <div className="subscription-renewal-card-heading">
                  <div className="subscription-renewal-small-icon orange"><FaExclamationTriangle /></div>
                  <div>
                    <h3>Renewal Urgency</h3>
                    <p>Payments requiring attention</p>
                  </div>
                </div>
                ...
                <div className="subscription-renewal-urgency-list">
                  <div className="renewal-urgency-item overdue">
                    <span><i />Overdue</span>
                    <strong>{overdueRenewals.length}</strong>
                  </div>
                  <div className="renewal-urgency-item today">
                    <span><i />Today</span>
                    <strong>{todayRenewals.length}</strong>
                  </div>
                  <div className="renewal-urgency-item tomorrow">
                    <span><i />Tomorrow</span>
                    <strong>{tomorrowRenewals.length}</strong>
                  </div>
                  <div className="renewal-urgency-item soon">
                    <span><i />Within 7 days</span>
                    <strong>{soonRenewals.length}</strong>
                  </div>
                </div>
              </div>
              ...
              <div className="subscription-renewal-auto">
                <div className="subscription-renewal-card-heading">
                  <div className="subscription-renewal-small-icon violet"><FaSyncAlt /></div>
                  <div>
                    <h3>Auto-Renew Exposure</h3>
                    <p>Automatic charges coming soon</p>
                  </div>
                </div>

                <div className="subscription-renewal-auto-value">
                  <strong>{autoRenewUpcoming.length}</strong>
                  <span>
                    auto-renewing subscription
                    {autoRenewUpcoming.length !== 1 ? "s" : ""} within 30 days
                  </span>
                </div>

                <div className="subscription-renewal-auto-amount">
                  <span>POTENTIAL AUTOMATIC SPEND</span>
                  <strong>
                    {formatCurrency(
            ...
            <div className="subscription-renewal-control-center">
              ...
              <div className="subscription-renewal-window-row">
                {[7, 30, 90].map((days) => (
                  <button
                    key={days}
                    type="button"
                    className={renewalWindow === days ? "active" : ""}
              ...
              <div className="subscription-renewal-control-summary">
                <div className="renewal-control-stat">
                  <div className="renewal-control-stat-icon blue"><FaCalendarAlt /></div>
                  <div>
                    <span>RENEWALS</span>
                    <strong>{renewalWindowSubscriptions.length}</strong>
                    <small>within selected window</small>
                  </div>
                </div>

                <div className="renewal-control-stat">
                  <div className="renewal-control-stat-icon orange"><FaWallet /></div>
                  <div>
                    <span>EXPECTED SPEND</span>
                    <strong>{formatCurrency(renewalWindowSpend)}</strong>
                    <small>scheduled charges</small>
                  </div>
                </div>

                <div className="renewal-control-stat">
                  <div className="renewal-control-stat-icon violet"><FaSyncAlt /></div>
                  <div>
                    <span>AUTO-RENEW</span>
                    <strong>{renewalWindowAutoRenew}</strong>
                    <small>automatic charges</small>
                  </div>
                </div>

                <div className="renewal-control-stat">
                  <div className="renewal-control-stat-icon green"><FaBell /></div>
                  <div>
                    <span>MANUAL REVIEW</span>
                    <strong>{renewalWindowManual}</strong>
                    <small>subscriptions to review</small>
                  </div>
                </div>
              </div>

              <div className="subscription-renewal-control-list">
                {renewalWindowSubscriptions.length === 0 ? (
                  <div className="subscription-renewal-control-empty">
                    <FaCheckCircle />
                    <div>
                      <strong>No renewals in this window</strong>
                      <span>Try a longer renewal window.</span>
                    </div>
                  </div>
                ) : (
                  renewalWindowSubscriptions.slice(0, 6).map((subscription) => {
                    const renewal = getRenewalInfo(subscription.nextPayment);
                    ...
                    return (
                      ...
                      >
                        <div className="subscription-renewal-control-date">
                          <strong>
                            {new Date(`${subscription.nextPayment}T00:00:00`).getDate()}
                          </strong>
                          <span>
                            {new Date(`${subscription.nextPayment}T00:00:00`).toLocaleDateString(
                              "en-IN",
                              { month: "short" }
                            )}
                          </span>
                        ...
                        <div className={`subscription-renewal-control-status ${renewal.className}`}>
                          {renewal.icon}
                        ...
                        <div className="subscription-renewal-control-actions">
                          <button
                            ...
                            className={subscription.autoRenew ? "auto-enabled" : ""}
                          >
                            <FaToggleOn />
                            <span>
            ...
            <div className="subscription-renewal-timeline">
              <div className="subscription-renewal-timeline-header">
                <div>
                  <h3>Upcoming Renewal Timeline</h3>
                  <p>Your next recurring charges in order.</p>
                </div>

                <span>
                  {renewalSubscriptions.length} active renewal
                  {renewalSubscriptions.length !== 1 ? "s" : ""}
                </span>
              </div>
              ...
              {renewalSubscriptions.length === 0 ? (
                <div className="subscription-renewal-timeline-empty">
                  <FaCheckCircle />
                  <span>No upcoming renewals to display.</span>
                </div>
              ) : (
                <div className="subscription-renewal-timeline-list">
                  {renewalSubscriptions.slice(0, 6).map((subscription) => (
                    <div
                      key={subscription.id}
                    ...
                    >
                      ...
                      <div className="subscription-renewal-timeline-status">
                        <span>
                          {subscription.renewalInfo.icon}
                          {subscription.renewalInfo.label}
                        </span>
                        <strong>{formatCurrency(subscription.amount)}</strong>
                      </div>
          ...
          <section className="subscription-toolbar">
            ...
            <div className="subscription-filters">
              {["All", "Active", "Paused", "Cancelled"].map((item) => (
                <button
                  type="button"
                  key={item}
            ...
            <div className="subscription-list-panel">
              ...
              {filteredSubscriptions.length === 0 ? (
                <div className="subscription-empty">
                  ...
                  <p>Add a subscription or change your search/filter.</p>
                ...
                <div className="subscription-list">
                  {filteredSubscriptions.map((subscription) => {
                    const type = subscription.status.toLowerCase();
                    const renewal = getRenewalInfo(subscription.nextPayment);
                    const isNextPayment = nextPayment?.id === subscription.id;

                    return (
                      <article
                        key={subscription.id}
                        className={`subscription-card ${type} ${
                          isNextPayment ? "next-payment-card" : ""
                        }`}
                      >
                        ...
                        <div className="subscription-card-content">
                          <div className="subscription-card-title">
                            ...
                            <div className={`subscription-status ${type}`}>
                              <span />
                              {subscription.status}
                            </div>
                          </div>
                          ...
                          <div className="subscription-card-details">
                            ...
                            <div>
                              <span>NEXT PAYMENT</span>
                              <strong>{formatDate(subscription.nextPayment)}</strong>
                            </div>
                            <div>
                              <span>PAYMENT</span>
                          ...
                          <div className="subscription-card-bottom">
                            <div className="subscription-renewal-info">
                              <span className={`subscription-days ${renewal.className}`}>
                                {renewal.icon}
                                {renewal.label}
                              </span>
            ...
            <aside className="subscription-upcoming-panel">
              <div className="subscription-section-header">
                <div>
                  ...
                  <p>Your next recurring payments</p>
                </div>
                <FaCalendarAlt />
              </div>
              ...
              {upcomingPayments.length === 0 ? (
                ...
                <div className="upcoming-payment-list">
                  {upcomingPayments.slice(0, 5).map((subscription) => {
                    ...
                    const renewal = getRenewalInfo(subscription.nextPayment);
                    const isNext = nextPayment?.id === subscription.id;

                    return (
                      <div
                        className={`upcoming-payment ${isNext ? "next-payment" : ""}`}
                        key={subscription.id}
                      >
                        <div className="upcoming-payment-date">
                          <strong>{paymentDate.getDate()}</strong>
                          <span>
                        ...
                        <div className="upcoming-payment-info">
                          <strong>{subscription.name}</strong>
                          <span className={`upcoming-renewal-status ${renewal.className}`}>
                            {renewal.label}
                          </span>
                        </div>
      ...
      {showModal && (
        ...
        >
          <form className="subscription-modal" onSubmit={saveSubscription}>
            <div className="subscription-modal-header">
              <div>
                <h2>{editingId ? "Edit Subscription" : "Add Subscription"}</h2>
                <p>Add your recurring payment details.</p>
              </div>
